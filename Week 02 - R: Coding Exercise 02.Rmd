---
title: "Week 02 Coding Exercise - Questions"
...
third_place <- function(names, score){
  ...
  df<-data.frame(names,score)
sorted<-sort(df$score[!duplicated(df$score)], decreasing = TRUE)  #menghilangkan duplikasi data score sebelum sortir
c1<-(which(df$score == sorted[3])) #mengecek mana saja baris yang sama di df asli dengan yang sudah di sortir
nameList <- c()
    for(row in 1:length(c1))
    {
      if (row==length(c1))
          nameList <- paste0(nameList, df[c1[row], 1])
              else 
          nameList <- paste0(nameList, df[c1[row], 1], ", ")
}
  print(paste("The third winner:",nameList))
}
...
square_root <- function(a, b, c){
   tryCatch( #untuk mendeteksi error karena tidak ada solusi
  {
  d<-sqrt(b^2-4*a*c)
  x1<--b+d/2*a
  x2<--b-d/2*a
...
paste0("X1 = ",x1,", X2 = ",x2)},
 warning=function(e){print("No solution")})
...
#start
distance<-function(p1,p2){
  z<-sqrt((p2[1]-p1[1])^2+(p2[2]-p1[2])^2) #euclidean distance formula
  print(paste("The distance is",z))
  }
distance(point_1,point_2)
```
